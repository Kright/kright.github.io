---
ref: uskoriaem-for-loop-v-scala-3
redirect_from:
  - /2026/06/27/%D0%A3%D1%81%D0%BA%D0%BE%D1%80%D1%8F%D0%B5%D0%BC-for-loop-%D0%B2-Scala-3.html
permalink: /2026/06/27/uskoriaem-for-loop-v-scala-3/
...
`0 until n` создаёт объект `Range`, у которого есть метод `foreach`. Но этот метод принимает лямбду с обобщённым типом `Int => Unit`, а значит вместо числа случится боксинг и передадут объект `java.lang.Integer`. JIT-компилятор обычно справляется для одного цикла, но если в коде несколько вложенных циклов, как при перемножении матриц, то случается фиаско с производительностью.
...
В компиляторе Kotlin есть оптимизации (lowerings) именно для этого случая. Компилятор ищет паттерны `for (i in 0..n)` и генерирует напрямую байткод, идентичный натуральному `while`.
